...
export default function InnovationPage() {
  return (
    <main className="pt-20">
              ...
              <Reveal key={i} delay={i * 0.1}>
                <div className="p-6 rounded-xl border border-gray-200 bg-white shadow-sm">
                  <div className="w-12 h-12 rounded-full bg-gradient-to-br from-cyan-400/20 to-blue-400/20 border border-cyan-400/30 mb-4 flex items-center justify-center">
                    {area.name === "소재 개발" && <Beaker className="text-green-600" />}
            ...
            <div className="p-8 rounded-2xl border border-gray-200 bg-gradient-to-br from-green-500/10 to-emerald-500/10">
              ...
              <div className="grid md:grid-cols-4 gap-6">
                <div className="text-center p-6 rounded-xl border border-gray-200 bg-white shadow-sm">
                  ...
                  <div className="text-sm text-gray-600 mb-1">등록 특허</div>
      ...
      <Section id="patents" title="핵심 특허 포트폴리오">
        <div className="grid lg:grid-cols-3 gap-8">
          {innovation.patents.map((patent, i) => (
            <Reveal key={i} delay={i * 0.1}>
              <div className="p-6 rounded-xl border border-gray-200 bg-white shadow-sm group hover:bg-white/10 transition-all">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className="text-xs bg-green-500/20 text-green-600 px-2 py-1 rounded-full">등록</span>
                </div>
            ...
            <table className="w-full border-collapse rounded-xl overflow-hidden">
              ...
              <tbody>
                {innovation.comparisonTable.rows.map((row, i) => (
                  <tr key={i} className="border-b border-gray-200 hover:bg-white shadow-sm transition-colors">
                    {row.map((cell, j) => (
                      <td key={j} className={`p-4 text-sm ${
                        j === 0 ? 'font-medium text-foreground' : 
                        j === 1 ? 'font-medium text-green-600 bg-cyan-500/5' : 
                        'text-gray-700'
                      }`}>
                        {cell}
                      </td>
      ...
      {/* 특허 인증 배너 - 무한 스크롤 */}
      <div className="py-20 bg-gradient-to-r from-cyan-900/20 via-blue-900/15 to-purple-900/20 border-y border-gray-200">
        <Reveal>
        ...
        <div className="grid md:grid-cols-3 lg:grid-cols-4 gap-6">
          {innovation.certifications.map((cert, i) => (
            <Reveal key={i} delay={i * 0.05}>
              <div className="p-4 rounded-xl border border-gray-200 bg-white shadow-sm text-center group hover:bg-white/10 transition-all">
      ...
      <Section id="roadmap" title="미래 기술 로드맵" subtitle="지속적인 혁신으로 미래를 준비합니다">
        <div className="space-y-12">
          {Object.entries(innovation.roadmap).map(([key, phase], i) => {
            const colors = [
              { bg: "from-green-500/10 to-emerald-500/10", icon: "text-green-600", border: "border-green-500/20" },
                ...
                <div className={`p-8 rounded-2xl border ${color.border} bg-gradient-to-br ${color.bg}`}>
                  <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                      ...
                      <p className="text-lg text-gray-600">{phase.period}</p>
                    </div>
                  </div>
                  
                  <div className="grid md:grid-cols-3 gap-4">
                    {phase.goals.map((goal, j) => (
                      <div key={j} className="p-4 rounded-xl border border-gray-200 bg-white shadow-sm">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <Reveal>
            <a href="/contact?type=partnership" className="group p-6 rounded-xl border border-gray-200 bg-white shadow-sm hover:bg-white/10 transition-all block text-center">
              ...
              <p className="text-sm text-gray-600">공동 연구개발 및 기술 협력</p>
            </a>
          </Reveal>
          
          <Reveal delay={0.1}>
            <a href="/contact?type=licensing" className="group p-6 rounded-xl border border-gray-200 bg-white shadow-sm hover:bg-white/10 transition-all block text-center">
              ...
              <p className="text-sm text-gray-600">특허 기술 라이센스 문의</p>
            </a>
          </Reveal>
          ...
          <Reveal delay={0.2}>
            <a href="/contact?type=consultation" className="group p-6 rounded-xl border border-gray-200 bg-white shadow-sm hover:bg-white/10 transition-all block text-center">
              ...
              <p className="text-sm text-gray-600">전문가 기술 자문 서비스</p>
            </a>
          </Reveal>
        </div>
      </Section>
